...
void dw_main (void const * argument)
{
	...
	if (!start){
		...
		//this is called here to wake up the device (i.e. if it was in sleep mode before the restart)
		uint32 devID = instancereaddeviceid() ;
		if(DWT_DEVICE_ID != devID) //if the read of device ID fails, the DW1000 could be asleep
		{
			port_wakeup_dw1000();
		...
		dwt_configuresleep((DWT_PRESRV_SLEEP|DWT_TANDV), DWT_WAKE_WK|DWT_SLP_EN);
		dwt_entersleep();
	...
	while(1)
	{
		/* get the local protocol strucure */
...
#ifdef LOW_POW_LISTENING
		/* testing !!!!!!!!!!!!!! */
		if ((inst->testAppState == TA_RX_WAIT_DATA) && (inst->mode == ANCHOR) && (inst->LPListenEabled == LPL_READY)){

			port_DisableEXT_IRQ(); 	//disable DW1000 IRQ until we configure the application

		    /* Configure sleep mode to allow low-power listening to operate properly. */
		    dwt_configuresleep(DWT_PRESRV_SLEEP | DWT_CONFIG | DWT_RX_EN,
		    		DWT_WAKE_SLPCNT | DWT_SLP_EN | DWT_WAKE_CS | DWT_WAKE_WK);
		    ...
		    /* LPListen disabled */
		    inst->LPListenEabled = LPL_RUNNING;
...
static void LPL_Exit (void){
	...
	if (inst->LPListenEabled == LPL_EXITING){
		port_DisableEXT_IRQ(); 	//disable DW1000 IRQ until we configure the application
		...
		/* runs the instance once */
		instance_run();

		inst->LPListenEabled = LPL_OFF;
