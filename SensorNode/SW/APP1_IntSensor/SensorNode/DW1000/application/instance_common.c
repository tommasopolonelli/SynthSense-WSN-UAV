...
float instance_get_PowerTransfer(void) //get average range
{
#ifdef COMBO_UWB_POWERTRANSF
    return instance_data[0].RX_PowerTransfer_V;
#else
    return 0;
#endif
}

void instance_set_PowerTransfer(float x) //get average range
{
#ifdef COMBO_UWB_POWERTRANSF
	instance_data[0].TX_PowerTransfer_V = x;
#else
	return;
#endif
}
...
void instance_rxgoodcallback(const dwt_cb_data_t *rxd)
{
	...
	/* are we in LPL ? */
	if (instance_data[instance].LPListenEabled == LPL_RUNNING){
		/* preamble detected */
		/* exit from LPL */
		dwt_configuresleep(DWT_PRESRV_SLEEP | DWT_CONFIG, DWT_WAKE_CS | DWT_SLP_EN);
		/* clear instance and settings */
		instance_data[instance].LPListenEabled = LPL_EXITING;
	...
	if (rxd_event == SIG_RX_UNKNOWN){
		// read only when it isn't an ack frame and when the module is in iot mode
		dwt_readrxdata((uint8 *)&dw_event.msgu.frame[0], rxd->datalength, 0);  // Read Data Frame
		instance_putevent(dw_event);

		/* start IoT mode */
		if(iot_mode == 0){
			if(IS_ENABLE(dw_event.msgu.rxmsg_ss.messageData)){
				iot_mode = 1;
			}
			if(IS_DISABLE(dw_event.msgu.rxmsg_ss.messageData)){
				run = 0;
			}
		}
	}
...
#ifdef COMBO_UWB_POWERTRANSF
					memcpy(&(instance_data[instance].msg.messageData[PTVR]), &instance_data[instance].TX_PowerTransfer_V, 4);
#endif
...
event_data_t* instance_getevent(int x)
{
	...
	// start DMA transfer
	memcpy_DMA ((uint32_t *)&instance_data[instance].dwevent[indexOut].msgu,
			(uint32_t *)&dw_event_g.msgu,
			sizeof(instance_data[instance].dwevent[indexOut].msgu));
...
{
	int instance = 0;
	memset(&instance_data[instance], 0, sizeof(instance_data));

	memset(&dw_event_g, 0, sizeof(event_data_t));
