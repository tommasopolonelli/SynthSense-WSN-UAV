...
#define R_NUM_ELEMENTS	(21)

sMedianFilter_t r_medianFilter;
static sMedianNode_t r_medianBuffer[R_NUM_ELEMENTS];

void MediaFilterInit( void )
{
	r_medianFilter.numNodes = R_NUM_ELEMENTS;
	r_medianFilter.medianBuffer = r_medianBuffer;

    MEDIANFILTER_Init(&r_medianFilter);
